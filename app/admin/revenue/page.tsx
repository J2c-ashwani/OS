            ...
            <div className="px-8 py-8 flex flex-col gap-8 pb-20">
                ...
                <div className="flex flex-wrap justify-between items-end gap-3">
                    ...
                    <div className="flex gap-3">
                        <button className="flex min-w-[84px] cursor-pointer items-center justify-center rounded-lg h-10 px-4 bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white text-sm font-bold border border-slate-200 dark:border-transparent hover:bg-slate-200 dark:hover:bg-slate-800 transition-all">
                            <Download size={16} className="mr-2" />
                            <span>Export Report</span>
                        </button>
                        <button className="flex min-w-[84px] cursor-pointer items-center justify-center rounded-lg h-10 px-4 bg-emerald-600 text-white text-sm font-bold shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 transition-all">
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                            ...
                            <div>
                                <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                        <TableRow
                                            initials="AC" name="Acme Corp" date="Oct 24, 2023" value="$45,000"
                                            tier="Enterprise" tierColor="indigo" status="Stable" statusColor="green"
                                        />
                                        <TableRow
                                            initials="GL" name="Global Logistics" date="Oct 28, 2023" value="$12,200"
                                            tier="Pro" tierColor="purple" status="At Risk" statusColor="orange" pulse
                                        />
                                        <TableRow
                                            initials="SV" name="Sky Ventures" date="Nov 02, 2023" value="$8,500"
                                            tier="SMB Basic" tierColor="slate" status="Likely" statusColor="green"
                                        />
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        ...
                        <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
                            ...
                            <div className="flex flex-col gap-4">
                                <RiskCard name="Nova Soft" score="24" desc="Engagement dropped 65% in last 14 days." />
                ...
                <div className="bg-slate-900 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
                    ...
                    <div className="flex gap-4 z-10">
                        <button className="px-6 py-3 rounded-xl bg-white text-slate-900 font-bold text-sm hover:bg-slate-100 transition-colors">Review Projections</button>
                        <button className="px-6 py-3 rounded-xl bg-emerald-600 text-white font-bold text-sm shadow-xl shadow-emerald-500/30 hover:bg-emerald-700 transition-all">Finalize Audit</button>
                    </div>
                </div>
            </div>
        </div>
...
function Tab({ label, active }: { label: string, active?: boolean }) {
    return (
        <div className={`flex flex-col items-center justify-center border-b-2 pb-3 pt-4 px-2 cursor-pointer transition-colors ${active ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-emerald-600'}`}>
            <p className="text-sm font-bold leading-normal tracking-[0.015em]">{label}</p>
        </div>
    )
}

function TableRow({ initials, name, date, value, tier, tierColor, status, statusColor, pulse }: any) {
    ...
    return (
        <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
            ...
            <td className="px-6 py-4 text-right">
                <button className="text-slate-400 hover:text-emerald-600"><MoreVertical size={18} /></button>
            </td>
        </tr>
    );
}
...
function RiskCard({ name, score, desc, isSecondary }: any) {
    return (
        <div className={`p-3 rounded-lg border flex flex-col gap-2 ${isSecondary ? 'bg-slate-50 dark:bg-slate-900/50 border-slate-100 dark:border-slate-800' : 'bg-orange-50 dark:bg-orange-900/10 border-orange-100 dark:border-orange-900/20'}`}>
            ...
            {!isSecondary && (
                <div className="flex gap-2 mt-1">
                    <button className="flex-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded text-[10px] font-bold text-emerald-600 shadow-sm hover:bg-emerald-600 hover:text-white transition-all">Send Note</button>
                    <button className="flex-1 bg-emerald-600 text-white py-1.5 rounded text-[10px] font-bold shadow-sm">Call Rep</button>
                </div>
            )}
        </div>
    )
