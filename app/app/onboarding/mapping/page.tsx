...
export default function MappingPage() {
    ...
    const handleScanAgain = () => {
        ...
        setTimeout(() => {
            setScanning(false);
            toast('Scan complete — 3 active streams detected');
        }, 2000);
    };

    const handleConfirmMapping = () => {
        toast('Mapping confirmed successfully');
    };

    const handleSaveAndContinue = () => {
        toast('Mapping saved — redirecting to dashboard');
    ...
    const handleConfirmStage = (stage: string) => {
        setStageConfirmed(prev => ({ ...prev, [stage]: true }));
        toast(`"${stage}" stage confirmed`);
    };

    const handleRejectStage = (stage: string) => {
        setStageConfirmed(prev => ({ ...prev, [stage]: false }));
        toast(`"${stage}" stage rejected`, 'info');
    };

    const handleAssignStream = () => {
        toast('Stream assignment coming soon', 'info');
    };
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="mb-8">
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-2 text-primary">
                        <BrainCircuit size={14} className={scanning ? 'animate-spin' : 'animate-pulse'} />
                        <p className="text-sm font-medium leading-normal">
                            {scanning ? 'Scanning your business channels...' : 'AI is learning your business structure from 12 active streams...'}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="flex flex-wrap justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            disabled={scanning}
                            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-lg font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-2 hover:bg-slate-50 transition-colors disabled:opacity-50"
                        >
                            <RefreshCw size={18} className={scanning ? 'animate-spin' : ''} /> {scanning ? 'Scanning...' : 'Scan Again'}
                        </button>
                        <button
                            onClick={handleConfirmMapping}
                            className="bg-primary text-white px-6 py-2 rounded-lg font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all"
                        >
                            Confirm Mapping
                        </button>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between group hover:border-primary transition-colors cursor-pointer">
                            <div className="flex items-center gap-3">
                                <div className="size-10 rounded-lg bg-orange-100 flex items-center justify-center text-orange-600"><MessageCircle size={20} /></div>
                                <div><p className="text-slate-900 dark:text-white font-bold text-sm">#sales-leads</p><p className="text-slate-500 text-xs">Slack Channel</p></div>
                            </div>
                            <GripVertical size={18} className="text-primary opacity-0 group-hover:opacity-100" />
                        </div>
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between group hover:border-primary transition-colors cursor-pointer">
                            <div className="flex items-center gap-3">
                                <div className="size-10 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600"><Mail size={20} /></div>
                                <div><p className="text-slate-900 dark:text-white font-bold text-sm">support@company.com</p><p className="text-slate-500 text-xs">GSuite Inbox</p></div>
                            </div>
                            <GripVertical size={18} className="text-primary opacity-0 group-hover:opacity-100" />
                        </div>
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between group hover:border-primary transition-colors cursor-pointer">
                            <div className="flex items-center gap-3">
                                <div className="size-10 rounded-lg bg-green-100 flex items-center justify-center text-green-600"><Database size={20} /></div>
                                <div><p className="text-slate-900 dark:text-white font-bold text-sm">Direct Deals</p><p className="text-slate-500 text-xs">HubSpot CRM</p></div>
                            </div>
                            <GripVertical size={18} className="text-primary opacity-0 group-hover:opacity-100" />
                        </div>
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between group hover:border-primary transition-colors cursor-pointer border-dashed">
                            <div className="flex items-center gap-3 text-slate-400">
                                <div className="size-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center"><PlusCircle size={20} /></div>
                ...
                <div className="hidden lg:flex flex-col justify-center items-center relative py-12">
                    <div className="absolute inset-0 flex flex-col justify-around py-16 pointer-events-none">
                        <div className="h-px w-full bg-slate-200 dark:bg-slate-800 relative"><div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_rgba(19,91,236,0.6)]"></div></div>
                        <div className="h-px w-full bg-slate-200 dark:bg-slate-800 relative opacity-40"><div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-slate-300"></div></div>
                        <div className="h-px w-full bg-slate-200 dark:bg-slate-800 relative"><div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_rgba(19,91,236,0.6)]"></div></div>
                    </div>
                    <div className="z-10 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4 rounded-full shadow-xl">
                        <div className="size-16 rounded-full bg-primary/10 flex items-center justify-center"><BrainCircuit size={32} className="text-primary" /></div>
                    </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-6">
                        {/* Stage 1 - Inquiry (already mapped) */}
                        <div className="bg-primary/5 dark:bg-primary/10 border-2 border-primary p-5 rounded-2xl relative">
                            <div className="absolute -left-3 top-1/2 -translate-y-1/2 size-6 rounded-full bg-primary text-white flex items-center justify-center"><Check size={12} /></div>
                        ...
                        <div className="bg-white dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 p-5 rounded-2xl hover:border-primary transition-colors group">
                            ...
                            <button
                                ...
                                className="w-full py-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold flex items-center justify-center gap-1 group-hover:bg-primary group-hover:text-white transition-all"
                            >
                                <PlusCircle size={14} /> Assign Stream
                            </button>
                        </div>
                        {/* Stage 3 - Sales (auto-suggested) */}
                        <div className={`border p-5 rounded-2xl ${stageConfirmed['Sales'] ? 'bg-primary/5 dark:bg-primary/10 border-primary' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}>
                            <div className="flex justify-between items-start mb-2">
                                <h4 className="text-slate-900 dark:text-white font-black text-lg">Sales</h4>
                                <span className={`text-[10px] font-bold uppercase ${stageConfirmed['Sales'] ? 'text-primary' : 'text-slate-400'}`}>
                                    {stageConfirmed['Sales'] ? 'Confirmed' : 'Auto-Suggested'}
                                </span>
                            </div>
                            ...
                            {!stageConfirmed['Sales'] && (
                                <div className="flex gap-2">
                                    <button onClick={() => handleConfirmStage('Sales')} className="flex-1 py-1.5 rounded-md bg-primary text-white text-[10px] font-bold uppercase">Confirm</button>
                                    <button onClick={() => handleRejectStage('Sales')} className="flex-1 py-1.5 rounded-md bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold uppercase">Reject</button>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {/* Footer */}
            <footer className="mt-auto bg-white dark:bg-background-dark border-t border-slate-200 dark:border-slate-800 p-6 -mx-6 mb-[-32px] pt-4">
                <div className="max-w-6xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex gap-4">
                        <button onClick={handleSkip} className="px-6 py-2 rounded-lg text-slate-600 dark:text-slate-400 font-bold hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Skip for now</button>
                        <button onClick={handleSaveAndContinue} className="px-8 py-2 rounded-lg bg-primary text-white font-bold shadow-lg shadow-primary/30 hover:scale-[1.02] transition-transform">Save & Continue</button>
                    </div>
                </div>
            </footer>
        </div>
