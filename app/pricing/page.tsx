...
const tiers = [
    {
        ...
        planId: process.env.NEXT_PUBLIC_PAYPAL_PLAN_ID_GROWTH || 'P-3RX065706M3469222L5IFM4I', // dummy sandbox plan
        highlighted: false,
        features: [
    ...
    },
    {
        ...
        planId: process.env.NEXT_PUBLIC_PAYPAL_PLAN_ID_ENTERPRISE || 'P-4RX065706M3469222L5IFM4I', // dummy sandbox plan
        highlighted: true,
        features: [
...
export default function PricingPage() {
    const handleApprove = (data: any, actions: any) => {
        // PayPal subscription successfully created
        console.log('Subscription completed', data);
        window.location.href = `/app/settings?session_id=${data.subscriptionID}`;
    };
                ...
                <main className="container mx-auto max-w-5xl px-6 py-16 flex-1">
                    ...
                    <div className="mb-16 text-center mt-12">
                        ...
                        <p className="text-xl text-slate-400 max-w-2xl mx-auto">
                            Stop losing revenue to slow response times. Get Agentic AI monitoring your funnels 24/7.
                        </p>
                    </div>
                    ...
                    {/* Pricing Grid */}
                    <div className="max-w-[1200px] mx-auto px-6 grid md:grid-cols-2 gap-8 mb-24">
                        {tiers.map((tier) => (
                            <div
                                key={tier.name}
                            ...
                            >
                                ...
                                {/* PayPal CTA */}
                                <div className="mb-8 z-0 relative min-h-[50px]">
                                    {tier.planId ? (
                                        <PayPalButtons
                                            ...
                                            onApprove={async (data, actions) => void handleApprove(data, actions)}
                                            style={{
                                                shape: 'pill',
                                                color: tier.highlighted ? 'blue' : 'black',
                                                layout: 'horizontal',
                                                label: 'subscribe'
                                            }}
                                        />
                                    ) : (
                                        <div className="text-sm text-center p-3 rounded-lg bg-red-50 text-red-600 border border-red-200">
                                            PayPal Plan ID not configured for {tier.name}
                                        </div>
                                    )}
                                </div>
